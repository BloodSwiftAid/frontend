        ...
        {filteredHospitals.map((hosp) => (
          ...
          >
            <div className="flex justify-between items-start mb-12">
              ...
              <div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 ${hosp.is_verified ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border border-amber-500/20'}`}>
                {hosp.is_verified ? <ShieldCheck size={12}/> : <Activity size={12}/>}
                {hosp.is_verified ? 'Verified' : 'Pending Review'}
              </div>
            </div>
            ...
            <div className="pt-6 border-t border-glass-border flex justify-between items-center">
              <button 
                onClick={(e) => { e.stopPropagation(); adminApi.toggleHospitalVerified(hosp.id).then(fetchData); }}
                className={`flex items-center gap-3 px-4 py-2 rounded-xl transition-all cursor-pointer ${hosp.is_verified ? 'bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500' : 'bg-glass border border-glass-border hover:border-emerald-500/50 text-text-muted hover:text-emerald-500'}`}
              >
                <ShieldCheck size={16} />
                <span className="text-[9px] font-black uppercase tracking-widest">{hosp.is_verified ? 'Authorized Node' : 'Verify Facility'}</span>
              </button>
                  ...
                  <div className="space-y-12 animate-fade-in">
                    <div className="p-12 bg-glass border border-glass-border rounded-[56px] space-y-12">
                      <div className="flex justify-between items-center pb-12 border-b border-glass-border">
                        ...
                        <button onClick={() => adminApi.toggleHospitalVerified(portalData.id).then(fetchData)} className={`px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center gap-3 ${portalData.is_verified ? 'bg-emerald-500 text-bg-dark shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'bg-glass border border-glass-border text-text-muted hover:text-emerald-500 hover:border-emerald-500/50'}`}>
                          <ShieldCheck size={16} />
