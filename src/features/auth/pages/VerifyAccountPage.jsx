...
import { ShieldCheck, Loader2, ArrowRight, Mail, Key, CheckCircle } from 'lucide-react';
import { authApi } from '../../../services/api';
import logo from '../../../assets/logo.png';
import toast from 'react-hot-toast';

const VerifyAccountPage = ({ theme }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let msg = 'Failed to verify account. Please check your network connection.';
      
      if (err.response?.data) {
        const data = err.response.data;
        msg = data.message || data.detail || data.error || (typeof data === 'string' ? data : msg);
        if (msg === 'Failed to verify account. Please check your network connection.' && typeof data === 'object') {
      ...
      <div className="w-full max-w-[520px] relative z-10 animate-fade-in">
        <div className="bg-card-bg/60 backdrop-blur-3xl border border-glass-border rounded-[40px] p-10 md:p-14 shadow-2xl">
          <div className="flex flex-col items-center text-center mb-12">
            ...
            <h1 className="text-4xl font-black text-text-primary tracking-tight mb-3 uppercase">Verify Account</h1>
            <p className="text-text-secondary text-lg">Enter your verification code to activate your account</p>
          </div>
          ...
          {success ? (
            <div className="flex flex-col items-center text-center animate-fade-in py-8">
              ...
              <h3 className="text-2xl font-black text-text-primary uppercase tracking-widest mb-4">Verified Successfully</h3>
              <p className="text-text-secondary mb-8">Your account has been securely verified. You can now access the SwiftAid platform.</p>
              <button
                onClick={() => navigate('/login')}
                className="w-full btn btn-primary py-5 rounded-2xl shadow-xl shadow-accent/20 flex justify-center items-center gap-3 group"
              >
                <span className="text-lg font-bold tracking-tight uppercase">Proceed to Login</span>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-text-secondary ml-1">Verification Code</label>
                <div className="relative group">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted group-focus-within:text-accent transition-colors">
              ...
              >
                {loading ? (
                  ...
                  <div className="flex items-center justify-center gap-3">
                    <span className="text-lg font-bold tracking-tight uppercase">Verify My Account</span>
                    <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
                  </div>
                )}
              </button>
          ...
          {!success && (
            <p className="text-center text-sm text-text-secondary mt-8">
              Already verified? <Link to="/login" className="text-accent font-bold hover:underline">Sign in</Link>
            </p>
          )}
        </div>
      </div>
